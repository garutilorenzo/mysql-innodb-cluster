...
setup_gr_args() {
	...
	[ -z "$GROUP_NAME" ] && GROUP_NAME="$(mysql_get_config 'group_replication_group_name' "$@")"
	...
	if [ ! -z "$BOOTSTRAP" ]; then
		...
		if [ ! -d "$DATADIR/mysql" ] || [ -z "$GROUP_SEEDS" ]; then
			...
			if [ -z "$GROUP_NAME" ]; then
				...
				echo "group_replication_group_name=$GROUP_NAME" >> /etc/mysql/mysql.conf.d/gr.cnf
			fi
	...
	GR_ARGS="$GR_ARGS --group_replication_local_address=$HOSTNAME:33061"
}
